...
export function CropEditor({ imageDataUrl, initialQuad, onConfirm, onRetry }: CropEditorProps) {
  ...
  const toDisplay = (p: Point) => ({ x: p.x * displaySize.w, y: p.y * displaySize.h });
  ...
  const getEventPos = (e: React.MouseEvent | React.TouchEvent, rect: DOMRect) => {
    if ("touches" in e) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      <div className="flex items-center justify-between px-4 py-3 bg-black/90">
        ...
        <p className="text-white text-sm font-semibold">Ajustar el documento</p>
        <button
      ...
      <div className="bg-black/70 text-center py-2">
        <p className="text-white/70 text-xs">Arrastre las esquinas para ajustar</p>
      </div>
        ...
        <div className="relative" style={{ width: displaySize.w, height: displaySize.h }}>
          ...
          <svg className="absolute inset-0 w-full h-full" style={{ overflow: "visible" }}>
            ...
            {[1, 2].map((i) => {
              ...
              return (
                <g key={i}>
                  <line x1={topX} y1={topY} x2={botX} y2={botY} stroke="rgba(79,142,247,0.4)" strokeWidth="1" />
                  <line x1={leftX} y1={leftY} x2={rightX} y2={rightY} stroke="rgba(79,142,247,0.4)" strokeWidth="1" />
                </g>
              );
            })}
            {pts.map((p, i) => (
              <g key={i}>
                <circle cx={p.x} cy={p.y} r={HANDLE_RADIUS} fill="transparent" style={{ cursor: "grab" }}
                  onMouseDown={handlePointerDown(i) as any}
                  onTouchStart={handlePointerDown(i) as any}
                />
                <circle cx={p.x} cy={p.y} r={10} fill="white" stroke="#4F8EF7" strokeWidth="3" style={{ pointerEvents: "none" }} />
                <circle cx={p.x} cy={p.y} r={4} fill="#4F8EF7" style={{ pointerEvents: "none" }} />
              </g>
            ))}
          </svg>
        </div>
